...
###################################################################
y_train, x_train    = D.get_data('train', 'theta', 'omega', sim=sim, downsample=True) 

# quick clean of data
finite = np.all(np.isfinite(y_train), axis=1)
...
###################################################################
# set prior on log Mg, log M*, log Vmax, log Z*, log R*
prior_low   = [6.8, 7.8, 1.0, -4., -4.]
prior_high  = [13.5, 12.5, 3.5, 0., 0.]
...
# Optuna Parameters
n_trials    = 1000
study_name  = 'qphi.%s.theta_omega' % sim 
n_jobs     = 1
